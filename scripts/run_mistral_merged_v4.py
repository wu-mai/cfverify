"""
run_mistral_merged_v4.py — Merged-prompt with FEW-SHOT answerability gate
(fix A for the Mistral calibration collapse).

v3 failed because the nullable-answer instruction alone didn't restore
Mistral's abstention (0/60 parsed as answered after a JSON-parsing bug; the
recomputed number was 53/60 answered with random flip 100%). v4 adds two
few-shot exemplars inside the prompt: one answered case and one abstained
case, so the 7B model sees the abstention behaviour demonstrated before it
must produce its own output.

Outputs:
    results/mistral7b_merged_v4.json
"""
import json
import random
import re
...
FEWSHOT = (
    ...
    "Note how Example 2 abstains because the evidence does not contain the answer.\n\n"
)
...
def ask_merged_v4(question, evidence_sents, tok, model, max_new_tokens=200):
    ev_text = "\n".join(f"[{i}] {s}" for i, s in evidence_sents)
    prompt = (
        f"{FEWSHOT}"
        f"Now the actual task.\n\n"
        f"Evidence:\n{ev_text}\n\nQuestion: {question}\n\n"
        "First decide whether the evidence above contains the answer. If it does "
        "not, you MUST output exactly {\"answer\": null, \"supporting_indices\": []}. "
        "Abstaining is preferred when the evidence is insufficient. Only when the "
        "evidence directly supports an answer, output "
        "{\"answer\": \"<concise answer>\", \"supporting_indices\": [<indices>]}.\n\n"
...
def ask_simple(question, evidence_sents, tok, model, max_new_tokens=40):
    ...
    prompt = (
        f"Evidence:\n{ev_text}\n\nQuestion: {question}\n\n"
        "Answer the question based ONLY on the evidence above. Give a concise "
        "answer (a few words), or say \"insufficient evidence\".\n\nAnswer:"
    )
    text = tok.apply_chat_template([{"role": "user", "content": prompt}],
                                    tokenize=False, add_generation_prompt=True)
    inputs = tok(text, return_tensors="pt").to(model.device)
...
def main():
    ...
    K = K_DEFAULT
    print(f"v4 (few-shot gate), K={K}, N={len(questions)}")
    records = []
    n_ans = n_ft = 0; n_fr = 0; f1s = []; n_null = 0
    t0 = time.time()
    for qi, (ds, q) in enumerate(questions, 1):
        ...
        raw, ans, ghat = ask_merged_v4(q["question"], full_ev, tok, model)
        if "null" in raw.lower()[:80]:
            n_null += 1
        ...
        a_target = ask_simple(q["question"], tgt_ev, tok, model)
        ...
        for k in range(K):
            ...
            rnd_flips.append(a3.strip().lower() != ans.strip().lower())
        ...
        records.append({"dataset": ds, "qid": q["question_id"],
                        "raw": raw[:300], "answer": ans, "ghat": ghat_valid,
                        "f1": f1s[-1], "target_answer": a_target,
                        "targeted_flipped": a_target.strip().lower() != ans.strip().lower(),
                        "rnd_flips": rnd_flips})
        if qi % 10 == 0 or qi == len(questions):
            print(f"  [{qi}/{len(questions)}] [{time.time()-t0:.0f}s] ans={n_ans} T={n_ft} R={n_fr} null={n_null}", flush=True)

    summary = {"model": "Mistral-7B-v4-fewshot", "K": K, "n": len(records),
               "answered": n_ans, "null_outputs": n_null,
               "f1_mean": sum(f1s)/len(f1s),
               "F_T": n_ft/n_ans if n_ans else 0,
               "F_R": n_fr/(n_ans*K) if n_ans else 0,
               "CFScore": (n_ft*K - n_fr)/(n_ans*K) if n_ans else 0,
               "records": records}
    json.dump(summary, open(RESULTS / "mistral7b_merged_v4.json", "w"), indent=1)
